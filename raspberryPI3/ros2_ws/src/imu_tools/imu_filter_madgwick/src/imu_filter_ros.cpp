      ...
      initialized_(false)
{
    ...
    if (constant_dt_ == 0.0)
    {
        RCLCPP_DEBUG(get_logger(), "Using dt computed from message headers");
    } else
    {
        RCLCPP_DEBUG(get_logger(), "Using constant dt of %f sec", constant_dt_);
    }
    ...
    RCLCPP_DEBUG(get_logger(), "Imu filter gain set to %f", gain);
    RCLCPP_DEBUG(get_logger(), "Gyro drift bias set to %f", zeta);
    ...
    RCLCPP_DEBUG(get_logger(), "Magnetometer bias values: %f %f %f", mag_bias_.x,
...
void ImuFilterMadgwickRos::imuCallback(ImuMsg::ConstSharedPtr imu_msg_raw)
{
    ...
    if (!initialized_)
    {
        RCLCPP_DEBUG(get_logger(), "First IMU message received.");
    ...
    const rcl_interfaces::msg::ParameterEvent::SharedPtr event)
{
    ...
    for (auto &changed_parameter : event->changed_parameters)
    {
        ...
        {
            RCLCPP_DEBUG(get_logger(), "Parameter %s set to %f", name.c_str(),
                        value.double_value);
            if (name == "gain")
            {
