...
bool interfaces__msg__joystick_order__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // record_path
    PyObject * field = PyObject_GetAttrString(_pymsg, "record_path");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->record_path = (Py_True == field);
    Py_DECREF(field);
  }
  {  // throttle
...
PyObject * interfaces__msg__joystick_order__convert_to_py(void * raw_ros_message)
{
  ...
  {  // record_path
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->record_path ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "record_path", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
